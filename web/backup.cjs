...
async function exportBundle(dataDir, workspaceRoot) {
  ...
  async function walk(root, relative) {
    for (const e of await fs.readdir(root, { withFileTypes: true })) {
      ...
      const abs = path.join(root, e.name),
        rel = relative + "/" + e.name;
      if (e.isSymbolicLink())
        throw Error("Backup refuses symbolic links: " + rel);
      if (e.isDirectory()) {
        directories.push(rel);
        if (directories.length > 20000) throw Error("Too many directories");
        await walk(abs, rel);
      } else if (e.isFile()) {
        const stat = await fs.stat(abs);
        size += stat.size;
        if (size > MAX || files.length >= 20000)
          throw Error("Backup exceeds 128 MiB / 20000 files");
  ...
  await walk(dataDir, "data");
  await walk(workspaceRoot, "workspaces");
  return gzip(
    Buffer.from(
      JSON.stringify({
...
function validateBundle(b) {
  if (
    b.format !== "jenny-web-backup" ||
    b.version !== 1 ||
    !Array.isArray(b.files) ||
    !Array.isArray(b.directories) ||
    b.files.length > 20000 ||
    b.directories.length > 20000
  )
  ...
  const valid = (p) => {
    if (
      typeof p !== "string" ||
      !/^(data|workspaces)\//.test(p) ||
      p.includes("\\") ||
      p
        .split("/")
        .some((x) => !x || x === "." || x === ".." || /[\x00-\x1f]/.test(x)) ||
      seen.has(p)
    )
  ...
  for (const f of b.files) {
    ...
    size += bytes.length;
    if (size > MAX || digest(bytes) !== f.sha256)
      throw Error("Backup checksum/size mismatch");
  }
  ...
  for (const p of seen) {
    ...
    parts.pop();
    while (parts.length) {
      if (filePaths.has(parts.join("/")))
        throw Error("File/directory conflict");
      parts.pop();
    }
  }
  return b;
...
async function restoreBundle(bytes, destination) {
  const b = validateBundle(
    JSON.parse(await gunzip(bytes, { maxOutputLength: MAX * 2 })),
  );
  // Destination must not exist: a restore cannot overwrite live data.
  await fs.mkdir(destination, { mode: 0o700 });
  try {
    for (const d of ["data", "workspaces", ...b.directories])
      await fs.mkdir(path.join(destination, d), {
        recursive: true,
        mode: 0o700,
      });
    for (const f of b.files) {
