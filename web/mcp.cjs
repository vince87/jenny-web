...
class MCP {
  ...
  async close() {
    ...
    try {
      await request(this.config.url, {
        method: "DELETE",
        headers,
        privateNetwork: this.config.privateNetwork === true,
  ...
  async rpc(method, params, signal, notification = false) {
    ...
    const r = await request(this.config.url, {
      method: "POST",
      headers,
      body: JSON.stringify({
        jsonrpc: "2.0",
    ...
    if ((r.headers["content-type"] || "").includes("text/event-stream")) {
      for (const event of r.text.split(/\r?\n\r?\n/)) {
        ...
        if (data) {
          ...
          if (value.id === id) message = value;
        }
      }
    } else message = JSON.parse(r.text);
    if (message?.id !== id || message.jsonrpc !== "2.0")
      throw Error("MCP response ID mismatch.");
    if (message.error)
      throw Error("MCP request failed (" + message.error.code + ").");
  ...
  async list(signal) {
    ...
    for (let page = 0; page < 5; page++) {
      ...
      tools.push(...r.tools);
      if (tools.length > 100) throw Error("MCP tool limit exceeded.");
      cursor = r.nextCursor;
