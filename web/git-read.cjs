...
async function gitRead(workspaces, name, action) {
  ...
  const root = path.join(workspaces.root, name),
  ...
  if (
    ...
    throw new Error("Repository Git locale non supportato.");
  for (const file of [
    "commondir",
    ...
    try {
      await fs.lstat(path.join(dir, file));
      throw new Error("Repository Git locale non supportato.");
    } catch (e) {
  ...
  if (!Object.hasOwn(commands, action))
    throw new Error("Azione Git non valida.");
