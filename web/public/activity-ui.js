...
function paintActivity() {
  ...
  if (!active) return;
  const label = session?.phase === 'thinking' ? 'Il modello sta pensando…' : session?.phase === 'writing' ? 'Jenny sta scrivendo…' : 'In attesa del modello…';
  const seconds = session?.startedAt ? Math.max(0, Math.floor((Date.now() - Date.parse(session.startedAt))/1000)) : 0;
