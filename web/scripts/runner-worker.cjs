...
async function execute(root,job) {
  ...
  let entries=0,sourceBytes=0;
  if(await fs.realpath(target)!==target || !(await fs.lstat(target)).isDirectory()) throw Error('Invalid workspace');
  ...
  async function scan(dir,depth=0) {
    ...
    for(const e of await fs.readdir(dir,{withFileTypes:true})) {
      ...
      if(e.isFile()){sourceBytes+=(await fs.stat(path.join(dir,e.name))).size;if(sourceBytes>64*1024*1024)throw Error("Runner source exceeds 64 MiB");}
      if(e.isSymbolicLink())throw Error('Workspace symlinks are not supported by runner');
      if(e.isDirectory()) await scan(path.join(dir,e.name),depth+1);
    }
  }
  await scan(target);
  const name='jenny-test-'+randomUUID(); const args=dockerArgs(root,job,name);
  let output='', bytes=0,limited=false;
  const child=spawn('docker',args,{stdio:['ignore','pipe','pipe']});
  const remove=()=>promisify(execFile)('docker',['rm','-f',name],{timeout:10000}).catch(()=>{});
  const timer=setTimeout(()=>{limited=true;child.kill('SIGKILL');},60000);
  const consume=b=>{bytes+=b.length;if(bytes>32000){limited=true;child.kill('SIGKILL');}else output+=b.toString();};
  child.stdout.on('data',consume);child.stderr.on('data',consume);
  try {
    const code=await new Promise((resolve,reject)=>{child.on('error',reject);child.on('close',resolve);});
    return {ok:code===0&&!limited,output:output+(limited?'\nStopped: time/output limit.':'')};
  } finally {clearTimeout(timer);await remove();}
}
async function main() {
  const base=process.env.JENNY_URL,token=process.env.JENNY_TOKEN,root=await fs.realpath(process.env.WORKSPACES_DIR || './workspaces');
  if(!base || !token)throw Error('Set JENNY_URL, JENNY_TOKEN and WORKSPACES_DIR');
  const api=async(route,body)=>{const r=await fetch(base+'/api'+route,{method:'POST',headers:{Authorization:'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify(body),signal:AbortSignal.timeout(15000)});if(!r.ok)throw Error('Jenny HTTP '+r.status);return r.json();};
  while(true) {
    const {job}=await api('/runner/claim',{});
    if(job) {
      let result;try {result=await execute(root,job);}catch(e){result={ok:false,output:e.message};}
      await api('/runner/complete',{id:job.id,lease:job.lease,...result});
    }
