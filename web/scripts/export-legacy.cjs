...
async function main() {
  ...
  if (!dataDir || !destination)
    throw Error(
      "Usage: node web/scripts/export-legacy.cjs DATA_DIR NEW_DIRECTORY",
    );
  ...
  try {
    ...
    for (const s of sessions) {
          ...
          if (!done.has(call.id)) {
            s.messages.push({
              ...
              content: JSON.stringify({
                error: "Cancelled for legacy export; no automatic execution.",
              }),
            });
            done.add(call.id);
          }
      ...
      s.partial = "";
      await fs.writeFile(
        path.join(destination, s.id + ".json"),
        JSON.stringify(s),
        { flag: "wx", mode: 0o600 },
      );
    }
