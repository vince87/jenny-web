...
async function main(){
 ...
 try {
  const workspace=path.join(root,'sample');await fs.mkdir(workspace,{mode:0o755});
  await fs.writeFile(path.join(workspace,'probe.cjs'),`
...
`);
  const result=await execute(await fs.realpath(root),{workspace:'sample',recipe:'terminal',command:'node probe.cjs'});
  assert.equal(result.ok,true,result.output);assert.match(result.output,/ISOLATED_TERMINAL_OK/);
  await assert.rejects(fs.access(path.join(workspace,'temporary-output')));
  console.log('Docker terminal: non-root, offline, read-only source and temporary writes verified.');
