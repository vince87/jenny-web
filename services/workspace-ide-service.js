...
} = require("./workspace-ide-image-reads");
const {
...
const {
  ...
  sameFileIdentity,
} = require("./workspace-root-operation");
...
class WorkspaceIdeService {
  constructor({
    ...
    this._trashItemImpl =
      typeof trashItemImpl === "function" ? trashItemImpl : null;
    ...
    this._showItemInFolderImpl =
      typeof showItemInFolderImpl === "function" ? showItemInFolderImpl : null;
    this._openPathImpl =
      typeof openPathImpl === "function" ? openPathImpl : null;
  ...
  _requireRoot() {
    const root = String(
      this._configService.getToolsWorkspaceRoot?.() || "",
    ).trim();
    if (!root) {
  ...
  async _resolveInsideRoot(relPath) {
    ...
    let realPath;
    try {
      realPath = await this._pathPolicy.assertInsideRoot(resolved, {
  ...
  async stat(payload = {}, existingOperation = null) {
    return this._withRootOperation(
      "read",
      payload,
      existingOperation,
      async (operation) => {
        ...
        let target;
        try {
          target = await this._rootOperations.resolveLeaf(
            operation.root,
            relPath,
            operation,
            {
  ...
  async readFileBase64(payload = {}) {
    ...
    const buffer =
      stats.size > IMAGE_READ_MAX_BYTES
        ? null
    ...
    if (observedSize > IMAGE_READ_MAX_BYTES) {
      throw workspaceFsError(
        ...
        `Image is too large to preview (${observedSize} bytes > ${IMAGE_READ_MAX_BYTES} bytes).`,
        {
          ...buildPathLogHint(relPath),
          size: observedSize,
          max_bytes: IMAGE_READ_MAX_BYTES,
        },
      );
    }
  ...
  async writeFile(payload = {}) {
    return this._withRootOperation(
      ...
      async (operation) => {
        ...
        const target = await this._rootOperations.resolveLeaf(
          root,
          relPath,
          operation,
          {
        ...
        if (!target.stats)
          this._normalizeRelPath(payload.path, { strictName: true });
        ...
        await this._rootOperations.runHook(
          "beforeLeafMutation",
          {
            kind: "writeFile",
            operation,
        ...
        const current = await this._rootOperations.revalidateLeaf(
          root,
          target,
          operation,
        );
        ...
        try {
          ...
          tempCreated = true;
          await this._rootOperations.revalidateParent(
            root,
            target.parent,
            operation,
          );
          const beforeReplace = await this._rootOperations.revalidateLeaf(
            root,
            target,
            operation,
          );
  ...
  async _revalidateDirectoryForList(resolved, operation) {
    if (resolved.token) {
      await this._rootOperations.revalidateLeaf(
        operation.root,
        resolved.token,
        operation,
      );
  ...
  async listDirectory(payload = {}, existingOperation = null) {
    return this._withRootOperation(
      ...
      existingOperation,
      async (operation) => {
        const resolved = await this._resolveDirectoryForList(
          payload.path,
          operation,
        );
        ...
        const checkLifecycle = () => {
          ...
          if (this._now() - state.startedAt >= maxDurationMs)
            return "time_limit";
          return null;
        };
        const checkStop = () => {
        ...
        try {
          for await (const dirent of iterateDirectoryEntries(
            this._fs,
            realPath,
            {
              onCleanupError: () =>
                this._log("WARN", "workspace_fs.directory_close_degraded", {
            ...
            if (
              kind === "directory" &&
              hideGenerated &&
              isGeneratedDirectoryName(dirent.name, {
                platform: this._platform,
              }) &&
              !isAmbiguousGeneratedDirectoryName(dirent.name, {
                platform: this._platform,
              })
            )
              continue;
            ...
            if (kind === "file") {
              ...
              if (
                isBuildManifestFileName(dirent.name, {
                  platform: this._platform,
                })
              )
        ...
        const visible =
          hideGenerated && hasBuildManifest
            ? pruneAmbiguousGeneratedEntries(classified, {
                platform: this._platform,
              })
            : classified;
        visible.sort((a, b) => {
        ...
        for (const entry of visible) {
          ...
          let mtimeMs = 0;
          try {
            const stats = await this._fs.lstat(
              this._path.join(realPath, entry.name),
            );
            this._rootOperations.assertCurrent(operation);
            size = stats.size;
            mtimeMs = stats.mtimeMs;
    ...
    return this._withRootOperation(
      ...
      async (operation) => {
        ...
        const ignorePolicy = await resolveWalkIgnorePolicy({
          gitService: this._gitServiceProvider?.(),
          signal: operation.signal,
          platform: this._platform,
    ...
    return this._withRootOperation(
      ...
      async (operation) => {
        ...
        await this._rootOperations.runHook(
          "beforeLeafMutation",
          {
            kind: kind === "file" ? "createFile" : "createDirectory",
            operation,
            root,
            target,
          },
  ...
  async createFile(payload = {}) {
    return this._createLeaf(payload, {
      kind: "file",
      create: (targetPath) =>
        this._fs.writeFile(targetPath, "", { encoding: "utf8", flag: "wx" }),
    });
  }
  ...
  async rename(payload = {}) {
    return this._withRootOperation(
      ...
      async (operation) => {
        ...
        const to = this._normalizeRelPath(payload.to, {
          strictName: true,
          relocationFrom: from,
        });
        const root = operation.root;
        const source = await this._rootOperations.resolveLeaf(
          root,
          from,
          operation,
          {
        ...
        const targetCurrent = await this._rootOperations.revalidateLeaf(
          root,
          target,
          operation,
        );
        ...
        if (sameIdentity) {
          let tempPath;
          while (!tempPath) {
            const candidate = this._path.join(
              target.parent.realPath,
          ...
          try {
            ...
            await this._fs
              .rename(tempPath, source.operationPath)
              .catch(() => {});
            throw error;
          }
        } else {
  ...
  async openInDefaultApp(payload = {}) {
    ...
    this._log(
      "INFO",
      "workspace_fs.open_in_default_app",
      buildPathLogHint(relPath),
    );
  ...
  async readPreChange(payload = {}) {
    ...
    this._log("INFO", "workspace_fs.read_pre_change", {
      ...
      ...(result.found === true
        ? {}
        : { reason: String(result.reason || "missing") }),
    });
  ...
  async searchInFiles(payload = {}, existingOperation = null) {
    ...
    try {
      return await this._withRootOperation(
        "read",
        payload,
        existingOperation,
        async (operation) => {
          ...
          const result = await searchWorkspaceFiles({
            ...
            resolveDirectory: async (relPath) => {
              ...
              return directory.stats?.isDirectory?.()
                ? directory.realPath
                : null;
            },
            resolveFile: async (relPath) => {
              const file = await this._rootOperations.resolveLeaf(
                root,
                relPath,
                operation,
              );
            ...
            now: this._now,
            onWarning: (reason) =>
              this._log("WARN", "workspace_fs.search_cleanup_degraded", {
                reason,
              }),
          });
