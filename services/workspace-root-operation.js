...
class WorkspaceRootOperationManager {
  constructor({
    ...
    this._rootContextProvider =
      typeof rootContextProvider === "function"
        ? rootContextProvider
        : () => null;
    this._fs = fs;
    this._path = path;
  ...
  acquire({ kind = "read", expectedGeneration = null } = {}) {
    ...
    if (
      ...
      return failAfterAcquire(
        this._rootTransitionError("root_context_invalid"),
      );
    }
    if (expectedGeneration !== null && expectedGeneration !== undefined) {
      if (
        ...
        return failAfterAcquire(
          this._staleGenerationError(expectedGeneration, context.generation),
        );
      }
    }
    const operation = {
  ...
  assertCurrent(operation) {
    if (!this.owns(operation))
      throw this._rootTransitionError("operation_invalid");
    let current;
    try {
  ...
  async prepareRoot(operation) {
    ...
    let stats;
    try {
      lexicalStats = await this._step(operation, () =>
        this._fs.lstat(configuredPath),
      );
      realPath = await this._step(operation, () =>
        this._fs.realpath(configuredPath),
      );
    ...
    if (!stats.isDirectory())
      throw this._rootInvalidError("root_not_directory");
  ...
  async revalidateRoot(root, operation) {
    ...
    let currentStats;
    try {
      currentLexicalStats = await this._step(operation, () =>
        this._fs.lstat(root.configuredPath),
      );
      currentRealPath = await this._step(operation, () =>
        this._fs.realpath(root.configuredPath),
      );
      currentStats = await this._step(operation, () =>
        this._fs.stat(currentRealPath),
      );
    } catch (error) {
      if (String(error?.code || "").startsWith("CMP-")) throw error;
      throw this._rootInvalidError("root_unavailable");
    ...
    if (
      !this._samePath(currentRealPath, root.realPath) ||
  ...
  async _inspectDirectory(root, lexicalPath, operation) {
    ...
    if (!this._isInside(root.realPath, realPath))
      throw this._outsideError("parent_outside_root");
    if (!stats.isDirectory())
      throw this._rootInvalidError("parent_not_directory");
  ...
  async revalidateParent(root, parent, operation) {
    ...
    const current = await this._inspectDirectory(
      root,
      parent.lexicalPath,
      operation,
    );
  ...
  async resolveLeaf(
    ...
    relPath,
    operation,
    { createParents = false, allowMissing = false, preserveLeaf = false } = {},
  ) {
    const segments = String(relPath || "").split("/");
    const leafName = segments[segments.length - 1];
    const parent = await this.ensureParent(root, relPath, operation, {
    ...
    if (!lexicalStats) {
      ...
      const containmentPath = /^[A-Za-z]:[\\/]/.test(leafName)
        ? this._path.join(parent.realPath, leafName.replace(":", "_"))
        : lexicalPath;
  ...
  async revalidateLeaf(root, leaf, operation) {
    ...
    const currentLexicalStats = await this._lstatOrNull(
      leaf.lexicalPath,
      operation,
    );
    ...
    if (
      !currentLexicalStats ||
      !sameFileIdentity(leaf.lexicalStats, currentLexicalStats)
    ) {
    ...
    try {
      currentRealPath = await this._step(operation, () =>
        this._fs.realpath(leaf.lexicalPath),
      );
      currentStats = await this._step(operation, () =>
        this._fs.stat(currentRealPath),
      );
    ...
    if (
      !this._isInside(root.realPath, currentRealPath) ||
